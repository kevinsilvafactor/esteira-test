...
function toLink(row: LinkRow): Link {
  return { id: row.id, code: row.code, originalUrl: row.original_url, createdAt: row.created_at, deletedAt: row.deleted_at }
}
...
export class LinkRepository {
  ...
  public create(input: CreateLinkInput): Link {
    ...
    ).run(input.code, input.originalUrl, input.createdAt)
